...
class AcousticDirectoryParser(threading.Thread):
    ...
    def __init__(
        ...
        super().__init__()
        #self.input_type = corpus_directory.split(':')[0]
        #self.corpus_directory = corpus_directory.split(':')[1]
    ...
    def run(self) -> None:
        ...
        if not os.path.isdir(self.corpus_directory):
            with open(self.corpus_directory,'r',encoding='utf8') as fin:
                for line in fin:
                    try:
                        file_name, wav_path, transcription_path = line.strip().split('\t')
                        relative_path = wav_path.split('/')[-2]
                        #relative_path = ''
                    #BAC009S0722W0495 /mnt/cfs/SPEECH/hp/mfa_change/data/spk2/BAC009S0722W0495.flac /mnt/cfs/SPEECH/hp/mfa_change/data/spk2/BAC009S0722W0495.txt spk2
            ...
            for root, _, files in os.walk(self.corpus_directory, followlinks=True):
                exts = find_exts(files)
                #identifiers={'BAC009S0723W0494'}, lab_files={'BAC009S0723W0494': 'BAC009S0723W0494.txt'}, textgrid_files={}, wav_files={}, other_audio_files={'BAC009S0723W0494': 'BAC009S0723W0494.flac'}
